...
const mint = async (recipientAddress, amount) => {
    ...
    console.log("MINTING SUI:", truncateAmount(amount), " IBT");

    const tx = new Transaction();
    tx.moveCall({
        target: `${SUI_PACKAGE_OBJECT_ID}::IBT::mint`,
        arguments: [
...
async function burn(senderAddress, amount, txDigest) {
    ...
    if (senderAddress !== SUI_DEPLOYER_ADDRESS) {
        ...
        const transferEvent = txDetails.events?.find(e => 
            e.type === `${SUI_PACKAGE_OBJECT_ID}::IBT_coin::CoinTransferToBurnEvent`
        );
...
async function executeDeployerBurn(coinId) {
    ...
    burnTx.moveCall({
        target: `${SUI_PACKAGE_OBJECT_ID}::IBT_coin::burn`,
        arguments: [
...
async function handleDeployerDirectBurn(amount) {
    ...
    const balance = await client.getBalance({
        ...
        coinType: `${SUI_PACKAGE_OBJECT_ID}::IBT_coin::IBT_COIN`,
    });
